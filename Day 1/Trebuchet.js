/* 
* SOLUTION
* Space time complexity of O(N*K);
* where N is the length of the calibration string
* where K is the number of individual calibration strings
* ANSWER = 54338
*/
...
const arrayOfCalibrationStrings = calibrationDocument.split('\n');

function sumOfFirstLastDigit(str) {
    let firstDigit = '';
    let lastDigit = '';
    ...
    for (let i = 0; i < str.length; i++) {
        ...
        if (!isNaN(Number(str[str.length-i-1])))
            if (lastDigit === '')
                lastDigit = str[str.length-1-i];
    }

    return Number(firstDigit + lastDigit);
}

const answer = arrayOfCalibrationStrings.reduce((acc, calibrationString) => {
    return acc + sumOfFirstLastDigit(calibrationString);
}, 0);
